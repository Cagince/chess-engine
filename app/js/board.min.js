var GameBoard, GeneratePosKey, PIECEINDEX, ParseFen, PrintBoard, PrintPieceLists, ResetBoard, SquareAttacked, UpdateListMaterial;
...
SquareAttacked = function(sq, side) {
  var direction, i, j, k, l, len, len1, len2, len3, m, piece, t_sq;
  ...
  for (j = 0, len = KnightDirections.length; j < len; j++) {
    i = KnightDirections[j];
    piece = GameBoard.pieces[sq + i];
    if (piece !== SQUARES.OFFBOARD && PieceCol[piece] === side && PieceKnight[piece] === BOOL.TRUE) {
      return true;
    }
  }
  for (k = 0, len1 = RookDirections.length; k < len1; k++) {
    direction = RookDirections[k];
    t_sq = sq + direction;
    piece = GameBoard.pieces[t_sq];
    while (piece !== SQUARES.OFFBOARD) {
      if (piece !== PIECES.EMPTY && PieceRookQueen[piece] === BOOL.TRUE && PieceCol[piece] === side) {
        true;
      } else {
        break;
      }
  ...
  for (l = 0, len2 = BishopDirections.length; l < len2; l++) {
    direction = BishopDirections[l];
    t_sq = sq + direction;
    piece = GameBoard.pieces[t_sq];
    while (piece !== SQUARES.OFFBOARD) {
      if (piece !== PIECES.EMPTY && PieceBishopQueen[piece] === BOOL.TRUE && PieceCol[piece] === side) {
        true;
      } else {
        break;
      }
  ...
  for (m = 0, len3 = KingDirections.length; m < len3; m++) {
    i = KingDirections[m];
    piece = GameBoard.pieces[sq + i];
